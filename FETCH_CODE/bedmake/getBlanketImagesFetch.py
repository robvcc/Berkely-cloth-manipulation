...
def creatingConCatImages(RGBImage, depthImage):
    ...
    # cX, cY = detectMarker(RGBImage)
    cX, cY = 50,100
    #******* draw contours and cross ********
    # cv2.drawContours(RGBImage, cnts[conto], -1, (0, 255, 0), 3)
    LL=10 #line length
    cv2.line(RGBImage,(cX-LL,cY),(cX+LL,cY),(0,255,0),thickness=2)
    cv2.line(RGBImage, (cX, cY-LL), (cX , cY+LL), (255, 0, 0), thickness=2)
    ...
    # cv2.waitKey(-1)
    # rospy.sleep(1)
def detectMarker(RGBImage):
    b, g, r = cv2.split(RGBImage)

    # # ************ clean data ***************
    # cv2.imshow("main",RGBImage)
    # cv2.waitKey(3)
    # d=raw_input(fileName)
    # if(d=='n'):
    #     os.rename(dirPath+fileName,'/home/ron/Desktop/'+fileName)
    # continue
    # # ****************************************
    ...
    _, contours, _p = cv2.findContours(bw, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def main():
    ...
    rospy.Subscriber("/head_camera/depth_registered/image_raw",
